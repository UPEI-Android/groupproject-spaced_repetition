...
class DialogBoxCreator {
  ...
  dynamic createDialogBox(BuildContext context, String type, Function callbackOnSubmit, [String questionsCourse = "null"]) {
    ...
    if (type == "course") {
          ...
          actions: [
            TextButton(onPressed: () {
              if(courseName.text.length == 0)
                {
                  SnackBarCreator().showSnackBar(context, "Field Empty, No Course Added!");
                  Navigator.of(context).pop();
                }else {
                callbackOnSubmit(courseName.text);
                Navigator.of(context).pop();
                SnackBarCreator().showSnackBar(context, "Course Added!");
              }
            }, child: Text("Submit"))
          ],
        );
      ...
      return showDialog(context: context, builder: (context) {
        return AlertDialog(
          ...
          actions: [
            TextButton(onPressed: () {
              if(questionText.text.length == 0 || questionAnswer.text.length == 0 || (questionText.text.length == 0 && questionAnswer.text.length == 0))
                {
                  SnackBarCreator().showSnackBar(context, "One or More Fields Empty, No Question Added!");
                  Navigator.of(context).pop();

                }else {
                callbackOnSubmit(questionsCourse, questionText.text, questionAnswer.text);
                Navigator.of(context).pop();
                SnackBarCreator().showSnackBar(context, "Question Added!");
              }
            }, child: Text("Submit"))
          ],
        );
